...
import traceback
import socket
import os,sys,time,inspect,threading,fire,glob
    ...
    if result == 0:  # 如果调用成功
        ...
        for i in range(entries_read.value):
            ...
            if  file_info.fi3_pathname.lower() ==  g_file or file_info.fi3_pathname.lower()==target_path.lower():
                lprint(f"Closing file: {file_info.fi3_pathname} (ID: {file_info.fi3_id})")
                # 关闭文件
                if netapi32.NetFileClose(None, file_info.fi3_id) == 0:
                    lprint("File closed successfully.")
                else:
                    lprint("Failed to close the file.")
...
def unlock_file(file=''):
    ...
    #lprint ('g_file',g_file,open_files[:2])
    for file_info in open_files:
        ...
        if file_info['pathname'].lower() ==  g_file or file_info['pathname'].lower()==file.lower():
            ...
            if detailed_info:
                lprint (file_info['pathname'].lower())
        
                lprint(f"详细信息 - ID: {detailed_info.fi3_id}, Path: {detailed_info.fi3_pathname}, User: {detailed_info.fi3_username}, Locks: {detailed_info.fi3_num_locks}")
                if is_file_locked(file_info['pathname']) :
                    try:
                        close_shared_file(file_info['pathname'])
                        #os.system(f"cmd /c net {detailed_info.fi3_id} /close")
                    except:
                        traceback.lprint_exc()
                    # try:
...
def unlock_special_path(file=''):
    open_files = get_open_files()
    server_name = None  # 替换为实际的服务器名称
    lprint ('file',file,len(open_files))
    processedFile=[]
    for file_info in open_files:
        if file_info['pathname'].lower() in processedFile:
            continue
        processedFile.append(file_info['pathname'].lower())
        #lprint<<file_info['pathname'].lower()<<file.lower()<<#lprint.end;
        #if os.path.samefile(file.lower(),file_info['pathname'].lower()):
        if file.lower() in file_info['pathname'].lower():
            lprint("该文件被占用")
            detailed_info = get_file_info(server_name, file_info['id'])
            if detailed_info:
                lprint (file_info['pathname'].lower())
